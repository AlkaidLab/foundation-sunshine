...
40. **第十七轮：虚拟屏模式/HDR 下发与 EDID 发布校验（2026-09-13）**：用户报告"客户端虚拟屏（独占）
     ...
     - **用户复测（pkgrel 55）追加发现**：分辨率已正确（EDID 首选=客户端模式），但实际跑在
       2400x1080@**144**（KDE 自选该分辨率下最高刷新率）。根因是 `filter_stale_devices()` 里
       `|| is_vdd_connector(...)` 会把**所有**虚拟屏条目从 modes/HDR map 中删掉（`09a49cf1` 的
       "EDID 保证模式"假设），客户端申请的 60 Hz 从未下发；Windows 是用 CCD 下发到 VDD 的，这一步才是
       真正的不一致（模式表内容本身与 Windows SETMODES 一致，2400x1080 也有 60/90/120/144 四项）。
       修复（`44b0d1ea`）：`filter_stale_devices()` 增加 `keep_id`，apply 路径保留本次配置的虚拟屏，
       原始/回滚记录仍剔除；并把 set_display_modes 失败后的容忍条件从"广告中有"改为"**当前**就是该
