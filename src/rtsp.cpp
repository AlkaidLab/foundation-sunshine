...
#pragma pack(pop)
  ...
  cmd_describe(rtsp_server_t *server, tcp::socket &sock, launch_session_t &session, msg_t &&req) {
    ...
    // Record encrypted request state for diagnostics.
  ...
  cmd_announce(rtsp_server_t *server, tcp::socket &sock, launch_session_t &session, msg_t &&req) {
    ...
    // Detect control-only sessions where no video or audio stream is set up.
    ...
    // Check that any required encryption is enabled. Control-only sessions skip
    // video/audio encryption validation.
    if (!session.control_only) {
      auto encryption_mode = net::encryption_mode_for_address(sock.remote_endpoint().address());
      if (encryption_mode == config::ENCRYPTION_MODE_MANDATORY &&
  ...
  start() {
    ...
    BOOST_LOG(info) << "Alkaid adapter boundary: "
                    << ALK_GAMESTREAM_HOST_RTSP_HANDSHAKE_ADAPTER_ID
                    << " handles GameStream handshake/capability/launch; "
                    << ALK_GAMESTREAM_HOST_ENET_CONTROL_TRANSPORT_ADAPTER_ID
