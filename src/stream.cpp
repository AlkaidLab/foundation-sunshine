...
#include "alkaidlab/clipboard_sync/clipboard_sync.h"
#include "alkaidlab/microphone_uplink/microphone_uplink.h"
#include "alkaidlab/gamestream_host_adapter/clipboard_wire_codec.h"
#include "alkaidlab/gamestream_host_adapter/gamestream_enet_control_transport_adapter.h"
#define ALK_GAMESTREAM_HOST_MEDIA_UDP_PRODUCT_PLATFORM_OPS 1
#include "alkaidlab/gamestream_host_adapter/gamestream_media_udp_transport_adapter.hpp"
#undef ALK_GAMESTREAM_HOST_MEDIA_UDP_PRODUCT_PLATFORM_OPS
#include "alkaidlab/gamestream_host_adapter/microphone_wire_codec.h"
#include "alkaidlab/gamestream_host_adapter/rescue_wire_codec.h"
#include "alkaidlab/gamestream_host_adapter/gamestream_rtsp_handshake_adapter.h"
#include "alkaidlab/session_runtime/transport_runtime.h"
#include "alkaidlab/session_runtime/transport_monitor.h"
#include "alkaidlab/session_runtime/continuity_facade.h"
#include "alkaidlab/session_handshake/session_handshake.h"
...
#include "alkaidlab/media_packetization/gamestream_rtp.h"
#include "alkaidlab/media_packetization/gamestream_media_packetization.h"
#include "alkaidlab/media_redundancy/gamestream_rs_fec.h"
...
#include "platform/common.h"
  ...
  static constexpr const char *kSunshineClipboardBackendProviderId = platf::clipboard::backend_provider_id;
#else
  static constexpr const char kSunshineClipboardBackendProviderId[] = "win32";
#endif

static constexpr const char kSunshineMicrophoneBackendProviderId[] = "windows-wasapi-sink";

#ifndef SS_CURSOR_PLANE_SOURCE_VERSION
#define SS_CURSOR_PLANE_SOURCE_MAGIC 0x43525353U
#define SS_CURSOR_PLANE_SOURCE_VERSION 1
typedef struct _SS_CURSOR_PLANE_SOURCE_V1 {
  std::uint32_t magic;
  std::uint16_t version;
  std::uint16_t size;
  std::uint32_t sourceWidth;
  std::uint32_t sourceHeight;
  std::uint32_t bitmapHash;
} SS_CURSOR_PLANE_SOURCE_V1, *PSS_CURSOR_PLANE_SOURCE_V1;
...
// poll surface. Legacy compatibility is selected by the Adapter profile.
  ...
  #define SS_RESCUE_PTYPE ALK_GAMESTREAM_HOST_RESCUE_PTYPE
#endif
...
namespace stream {
  using clipboard_sync_ptr = std::unique_ptr<AlkClipboardSync, void (*)(AlkClipboardSync *)>;

  enum class clipboard_read_status_e : std::uint8_t {
    none = 0,
    item = 1,
    skipped = 2,
    failed = 3,
...
#pragma pack(push, 1)
  ...
  using video_packet_raw_t = AlkGamestreamVideoPacketRaw;
  using video_packet_enc_prefix_t = AlkGamestreamVideoEncryptionPrefix;
  using audio_packet_t = AlkGamestreamAudioPacket;

  struct control_header_v2: AlkGameStreamHostControlHeader {
    uint8_t *
    payload() {
      return alk_gamestream_host_control_payload(this);
    }
  };
  ...
  typedef struct control_encrypted_t: AlkGameStreamHostEncryptedControlHeader {
    uint8_t *
    payload() {
      return alk_gamestream_host_encrypted_control_payload(this);
    }
    // encrypted control_header_v2 and payload data follow
  } *control_encrypted_p;

  using audio_fec_packet_t = AlkGamestreamAudioFecPacket;

  struct mic_packet_t {
    AlkGamestreamRtpPacket rtp;
  };
...
#pragma pack(pop)
  ...
  static void
  record_network_transport_datagram(session_t *session, std::uint8_t channel, std::uint32_t flags, std::size_t length, bool tx);
  static void
...
#else
      (void)address_family;
      BOOST_LOG(error) << "Legacy GameStream control carrier is disabled; use network.transport/gamestream-enet native IO";
      return -1;
#endif
    }
  ...
  struct session_t {
    ...
    session_runtime::transport_path_t active_transport_path { session_runtime::make_provider_datagram_direct_transport_path() };
    ...
    AlkTransportMonitor transport_monitor {};
    AlkTransportMonitorHistory transport_history {};
    AlkTransportMonitorTuningSnapshot transport_tuning {};
    std::uint64_t transport_monitor_last_refresh_ms { 0 };
    AlkHandshakeSelection handshake_selection {};
    ...
    struct {
      ...
      std::uint32_t transport_compatibility_mode { ALK_GAMESTREAM_HOST_ENET_CONTROL_TRANSPORT_COMPAT_NATIVE_PRIMARY };
      ...
      struct {
        ...
        clipboard_sync_ptr sync { nullptr, &alk_clipboard_sync_destroy };
        std::vector<std::uint8_t> staged_item_data;
        std::string staged_mime_type;
        std::string staged_name;
        clipboard_read_status_e last_read_status { clipboard_read_status_e::none };
      } clipboard;
    } control;
    ...
    stream_quality::link_quality_state_e last_stream_quality_link_quality_state {
      stream_quality::link_quality_state_e::unknown,
    };
    stream_quality::scene_class_e last_stream_quality_scene_class {
      stream_quality::scene_class_e::static_scene,
    };
    std::uint32_t last_dynamic_clarity_flags { 0 };
    int last_dynamic_clarity_qp { 0 };
    int last_dynamic_clarity_bitrate { 0 };
    AlkFrameInterestObservation frame_interest_observation {};
    std::chrono::steady_clock::time_point last_frame_interest_observation_log {};
    ...
    struct {
      ...
      std::uint32_t path_lan_direct_windows { 0 };
      std::uint32_t path_identity_confident_windows { 0 };
    } stream_quality_diag;
  ...
  startup_path_evidence_for_launch_session(const rtsp_stream::launch_session_t &launch_session) {
    return {
      ...
      .preflight_available = launch_session.path_preflight_available,
      .preflight_observed_peer_endpoint = launch_session.path_preflight_observed_peer_endpoint,
      .preflight_host_local_endpoint = launch_session.path_preflight_host_local_endpoint,
      .preflight_rtt_ms = launch_session.path_preflight_rtt_ms,
      .preflight_age_ms = launch_session.path_preflight_age_ms,
    };
  }
  ...
  log_startup_path_decision(std::uint64_t runtime_id,
    ...
    BOOST_LOG(info) << "[session:path] evidence runtime=" << runtime_id
                    ...
                    << " preflight=" << (session_runtime::startup_path_preflight_is_fresh(evidence) ? 1 : 0)
                    << " preflightPeer=" << evidence.preflight_observed_peer_endpoint
                    << " preflightHostLocal=" << evidence.preflight_host_local_endpoint
                    << " preflightRttMs=" << evidence.preflight_rtt_ms
                    << " preflightAgeMs=" << evidence.preflight_age_ms
  ...
  video_shard_frame_boundary_flags(std::uint32_t block_index,
    ...
    if (block_count == 0 || data_shards == 0 ||
        block_index >= block_count || shard_index >= data_shards) {
      return { false, false };
    }

    const bool frame_start = block_index == 0 && shard_index == 0;
    const bool frame_end = block_index + 1 == block_count &&
                           shard_index + 1 == data_shards;
  ...
  static void
  refresh_transport_monitor_views(session_t *session, bool force = false) {
    if (!session) {
      return;
    }
    constexpr std::uint64_t refresh_interval_ms = 100;
    const auto now_ms = steady_clock_ms();
    if (!force &&
        session->transport_monitor_last_refresh_ms != 0 &&
        now_ms >= session->transport_monitor_last_refresh_ms &&
        now_ms - session->transport_monitor_last_refresh_ms < refresh_interval_ms) {
      return;
    }
    session->transport_monitor_last_refresh_ms = now_ms;
    alk_transport_monitor_history(&session->transport_monitor, &session->transport_history);
    alk_transport_monitor_tuning_snapshot(&session->transport_monitor, &session->transport_tuning);
  }

  static void
  observe_transport_channel_snapshot(session_t *session, std::uint32_t channel_kind) {
    if (!session) {
      return;
    }
    AlkTransportChannelStats channel_stats;
    alk_transport_channel_stats_init(&channel_stats);
    if (alk_transport_runtime_channel_stats(&session->control.transport_binding,
                                            channel_kind,
                                            &channel_stats)) {
      alk_transport_monitor_observe_channel(&session->transport_monitor,
                                            &channel_stats,
                                            steady_clock_ms());
      refresh_transport_monitor_views(session);
    }
  }


  static AlkGameStreamHostEnetControlTransportCompatibilityMode
  select_network_transport_compatibility_mode(const session_t *session) {
    AlkGameStreamHostEnetControlTransportCompatibilityRequest request;
    alk_gamestream_host_enet_control_transport_compatibility_request_init(&request);
...
#if ALKAID_TRANSPORT_NATIVE_PRIMARY_ENABLED
    request.flags |= ALK_GAMESTREAM_HOST_ENET_CONTROL_TRANSPORT_COMPAT_ALLOW_NATIVE_PRIMARY |
                     ALK_GAMESTREAM_HOST_ENET_CONTROL_TRANSPORT_COMPAT_ROLLBACK_GUARD_AVAILABLE;
#endif
    return alk_gamestream_host_enet_control_transport_compatibility_mode(&request);
  }

  static bool
  transport_uses_native_primary(const session_t *session) {
    return session && alk_gamestream_host_enet_control_should_use_native(
      static_cast<AlkGameStreamHostEnetControlTransportCompatibilityMode>(session->control.transport_compatibility_mode));
  }
  ...
  transport_compatibility_mode_name(const session_t *session) {
    ...
    return alk_gamestream_host_enet_control_transport_compatibility_mode_name(
      static_cast<AlkGameStreamHostEnetControlTransportCompatibilityMode>(session->control.transport_compatibility_mode));
  }

  static const char *
  transport_fallback_reason_name(const session_t *session) {
    if (!session) {
      return "missing-session";
    }
    return alk_gamestream_host_enet_control_transport_fallback_reason_name(
      static_cast<AlkGameStreamHostEnetControlTransportCompatibilityMode>(session->control.transport_compatibility_mode));
  }
  ...
  transport_phase_name(const session_t *session) {
    ...
    return alk_gamestream_host_enet_control_transport_phase_name(
      static_cast<AlkGameStreamHostEnetControlTransportCompatibilityMode>(session->control.transport_compatibility_mode));
  }
  ...
  record_frame_interest_feedback(void *channel_data, const video::frame_interest_feedback_t &feedback) {
    ...
    if (feedback.has_platform_observation &&
        feedback.platform_observation.version == ALK_FRAME_INTEREST_VERSION) {
      session->frame_interest_observation = feedback.platform_observation;
      const auto now = std::chrono::steady_clock::now();
      if (session->last_frame_interest_observation_log.time_since_epoch().count() == 0 ||
          now - session->last_frame_interest_observation_log >= 1000ms) {
        session->last_frame_interest_observation_log = now;
        BOOST_LOG(info) << "Alkaid frame-interest observation runtime=" << session->identity.runtime_id
                        << " flags=0x" << std::hex << feedback.platform_observation.flags << std::dec
                        << " intent=0x" << std::hex << feedback.platform_observation.intent_flags << std::dec
                        << " roi=" << feedback.platform_observation.roi_rect_count
                        << " dirty=" << feedback.platform_observation.dirty_rect_count
                        << " move=" << feedback.platform_observation.move_rect_count
                        << " accepted=" << feedback.platform_observation.accepted
                        << " roiAccepted=" << feedback.platform_observation.roi_accepted
                        << " fallback=0x" << std::hex << feedback.platform_observation.fallback_reason_flags << std::dec
                        << " primaryFallback="
                        << alk_frame_interest_fallback_reason_name(feedback.platform_observation.primary_fallback_reason)
                        << " provider=" << feedback.platform_observation.provider_id
                        << " backend=" << feedback.platform_observation.backend_id;
      }
    }
  }
  ...
  static void
  annotate_feedback_with_path_identity(session_t *session,
                                       stream_quality::feedback_t &feedback) {
    if (!session) {
      return;
    }

    const auto &path = session->active_transport_path;
    feedback.path_lan_direct =
      path.path_identity_kind == LI_SESSION_PATH_IDENTITY_TRUE_LAN &&
      path.route == session_runtime::transport_route_e::lan_direct &&
      path.startup_class == LI_SESSION_STARTUP_CLASS_LAN_FAST;
    feedback.path_identity_confident =
      path.identity_confidence_ppm >= 700000U ||
      (feedback.path_lan_direct && session->startup_path_decision.allow_lan_fast_start);
  }
  ...
  annotate_feedback_with_transport_stats(session_t *session,
    ...
    annotate_feedback_with_path_identity(session, feedback);
    ...
    apply_transport_stats_to_quality_feedback(stats, feedback);
  ...
  record_stream_quality_feedback_diag(session_t *session,
    ...
    const auto continuity_diagnosis =
      diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
        *diagnosis :
        stream_quality::diagnose_feedback(feedback);
    ...
    diag.path_lan_direct_windows += feedback.path_lan_direct ? 1U : 0U;
    diag.path_identity_confident_windows += feedback.path_identity_confident ? 1U : 0U;
    ...
    message << "Stream-quality diag runtime=" << session->identity.runtime_id
            ...
            << " pathLan=" << diag.path_lan_direct_windows
            << " pathConf=" << diag.path_identity_confident_windows
            ...
            << " diagnosisDomain=" << stream_quality::continuity_domain_name(continuity_diagnosis.primary_domain)
            << " secondaryDomain=" << stream_quality::continuity_domain_name(continuity_diagnosis.secondary_domain)
            << " confidence=" << continuity_diagnosis.confidence_ppm
            << " evidenceBits=0x" << std::hex << continuity_diagnosis.evidence_bits << std::dec
            << " allowedActions=0x" << std::hex << continuity_diagnosis.allowed_action_mask << std::dec
            << " forbiddenActions=0x" << std::hex << continuity_diagnosis.forbidden_action_mask << std::dec
  ...
  static void
  apply_stream_quality_action(session_t *session,
                              const stream_quality::action_t &action,
                              const char *source,
                              const AlkContinuityDiagnosisFrame *diagnosis) {
    if (!session) {
    ...
    const bool startup_guard =
      session->stream_quality_startup_guard_until.time_since_epoch().count() != 0 &&
    ...
    const stream_quality::runtime_action_context_t runtime_action_context {
    ...
    const auto raw_plan = stream_quality::plan_runtime_action(action, runtime_action_context);
    const auto plan =
      diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
        stream_quality::plan_runtime_action_with_diagnosis(action, runtime_action_context, *diagnosis) :
        raw_plan;
    const bool domain_gate_suppressed =
      (raw_plan.apply_bitrate && !plan.apply_bitrate) ||
      (raw_plan.apply_fec && !plan.apply_fec) ||
      (raw_plan.apply_fps && !plan.apply_fps) ||
      (raw_plan.apply_profile && !plan.apply_profile) ||
      (raw_plan.apply_idr && !plan.apply_idr);
    const auto sdk_decision = stream_quality::to_sdk_decision(action);
    const auto sdk_plan = stream_quality::plan_runtime_action_sdk(action, runtime_action_context);
    auto gated_sdk_plan = sdk_plan;
    if (gated_sdk_plan.version == ALK_STREAM_QUALITY_CONTROL_VERSION) {
      if (!plan.apply_bitrate) {
        gated_sdk_plan.flags &= ~ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_APPLY_BITRATE;
      }
      if (!plan.apply_fec) {
        gated_sdk_plan.flags &= ~ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_APPLY_FEC;
      }
      if (!plan.apply_fps) {
        gated_sdk_plan.flags &= ~ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_APPLY_FPS;
      }
      if (!plan.apply_profile) {
        gated_sdk_plan.flags &= ~ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_APPLY_PROFILE;
      }
      if (!plan.apply_idr) {
        gated_sdk_plan.flags &= ~ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_APPLY_IDR;
      }
      if (!plan.update_runtime_totals) {
        gated_sdk_plan.flags &= ~ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_UPDATE_RUNTIME_TOTALS;
      }
      if (plan.dynamic_apply_blocked) {
        gated_sdk_plan.flags |= ALK_STREAM_QUALITY_RUNTIME_ACTION_FLAG_DYNAMIC_APPLY_BLOCKED;
      }
    }
    alk_transport_monitor_observe_weak_network_decision(&session->transport_monitor,
                                                        &sdk_decision,
                                                        &gated_sdk_plan,
                                                        steady_clock_ms());
    AlkContinuityActionFrame continuity_action_frame;
    alk_continuity_action_frame_init(&continuity_action_frame);
    const bool continuity_action_frame_valid =
      diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION &&
      alk_continuity_action_frame_from_quality_decision_with_diagnosis(&sdk_decision,
                                                                       &gated_sdk_plan,
                                                                       diagnosis,
    ...
    int target_fec_percentage = plan.target_fec_percentage;
    int target_encoding_bitrate = plan.target_bitrate_kbps;
    int target_fps = plan.target_fps;
    int target_total_bitrate = plan.target_total_bitrate_kbps;
    int pacing_total_bitrate = plan.pacing_total_bitrate_kbps;
    ...
    bool apply_bitrate = plan.apply_bitrate;
    bool apply_fec = plan.apply_fec;
    bool apply_fps = plan.apply_fps;
    bool apply_profile = plan.apply_profile;
    const bool profile_deferred = plan.profile_deferred;
    const bool startup_fec_floor_blocked =
      apply_fec &&
      stream_quality::startup_fec_floor_should_block_decrease(startup_guard,
                                                              session->last_applied_stream_quality_fec,
                                                              target_fec_percentage);
    if (startup_fec_floor_blocked) {
      target_fec_percentage = session->last_applied_stream_quality_fec;
      target_total_bitrate = total_video_bitrate_from_encoding_bitrate(target_encoding_bitrate,
                                                                       target_fec_percentage);
      pacing_total_bitrate = std::max(pacing_total_bitrate, target_total_bitrate);
      apply_fec = false;
    }
    ...
    const auto fps_cooldown_suffix = plan.fps_apply_cooldown_ms > 0 ?
                                       ...
                                       std::string {};
    if (!dynamic_apply_blocked && plan.update_runtime_totals) {
    ...
    BOOST_LOG(info) << "Stream-quality controller [" << source << "] runtime="
                    ...
                    << " diagnosisDomain="
                    << (diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
                          stream_quality::continuity_domain_name(diagnosis->primary_domain) : "unknown")
                    << " secondaryDomain="
                    << (diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
                          stream_quality::continuity_domain_name(diagnosis->secondary_domain) : "unknown")
                    << " confidence="
                    << (diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
                          diagnosis->confidence_ppm : 0U)
                    << " evidenceBits=0x" << std::hex
                    << (diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
                          diagnosis->evidence_bits : 0U) << std::dec
                    << " allowedActions=0x" << std::hex
                    << (diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
                          diagnosis->allowed_action_mask : 0U) << std::dec
                    << " forbiddenActions=0x" << std::hex
                    << (diagnosis && diagnosis->version == ALK_CONTINUITY_FACADE_VERSION ?
                          diagnosis->forbidden_action_mask : 0U) << std::dec
                    ...
                    << ",profile=" << (apply_profile ? 1 : 0) << ")"
                      ...
                      << (startup_fec_floor_blocked ? " startupFecFloor=1" : "")
                    ...
                    << (domain_gate_suppressed ? " domainGate=1" : "")
  ...
  static stream_quality::action_t
  process_stream_quality_feedback(session_t *session,
                                  const stream_quality::feedback_t &feedback,
                                  const char *source) {
    const auto diagnosis = stream_quality::diagnose_feedback(feedback);
    auto action = session->stream_quality_controller.on_feedback(feedback);
    apply_stream_quality_action(session, action, source, &diagnosis);
    record_stream_quality_feedback_diag(session, feedback, action, source, &diagnosis);
    return action;
  }
  ...
  report_stream_quality_recovery_request(session_t *session, const char *source) {
    ...
    process_stream_quality_feedback(session, feedback, source);
  }
  ...
  should_hold_startup_stream_quality_feedback(session_t *session,
    ...
    const bool startup_empty_no_delivery =
      stream_quality::startup_feedback_is_empty_no_delivery(feedback);
    ...
    if (!startup_empty_no_delivery &&
        (loss_impairs_delivery || visual_stale || startup_no_display || severe_client_backpressure)) {
      return false;
    }
    ...
    if (session->last_stream_quality_startup_guard_log.time_since_epoch().count() == 0 ||
      ...
      BOOST_LOG(info) << "Stream-quality startup guard holding transient feedback"
                      ...
                      << " emptyNoDelivery=" << (startup_empty_no_delivery ? 1 : 0)
  ...
  namespace clipboard_payload {
    ...
    build_item_start(uint8_t transfer_flags,
      ...
      const auto payload_size = alk_gamestream_host_clipboard_item_start_size(mime_type.size(), name.size());
      std::vector<uint8_t> payload(payload_size);
      std::size_t payload_length = 0;
      if (!alk_gamestream_host_clipboard_build_item_start(transfer_flags,
                                                   item_type,
                                                   item_id,
                                                   content_hash,
                                                   total_length,
    ...
    build_item_chunk(std::uint64_t item_id,
      ...
      std::size_t payload_length = 0;
      if (!alk_gamestream_host_clipboard_build_item_chunk(item_id,
                                                   chunk_offset,
    ...
    build_item_end(std::uint64_t item_id) {
      ...
      std::size_t payload_length = 0;
      (void) alk_gamestream_host_clipboard_build_item_end(item_id,
                                                   payload.data(),
                                                   payload.size(),
                                                   &payload_length);
      return payload;
  ...
  ensure_microphone_uplink_module(broadcast_ctx_t &ctx) {
    ...
    BOOST_LOG(info) << "Alkaid runtime marker: using Alkaid SDK microphone-uplink module"
                    ...
                    << " backendProvider=" << kSunshineMicrophoneBackendProviderId
  ...
  control_server_t::get_native_session(std::uint64_t peer_id,
    ...
    BOOST_LOG(info) << "Alkaid adapter boundary: " << ALK_GAMESTREAM_HOST_ENET_CONTROL_TRANSPORT_ADAPTER_ID << " active"
                    << " runtime=" << session->identity.runtime_id
                    << " launchSession=" << session->launch_session_id
                    << " nativePeerId=" << peer_id
                    << " peer=" << (remote_endpoint_str.empty() ? "unknown" : remote_endpoint_str)
                    << " rtspLaunchAdapter=" << ALK_GAMESTREAM_HOST_RTSP_HANDSHAKE_ADAPTER_ID << "-detached"
  ...
  struct control_peer_health_snapshot_t {
    std::uint32_t last_recv_age { 0 };
    std::uint32_t last_send_age { 0 };
    std::uint32_t next_timeout_in { 0 };
  ...
  alk_transport_protocol_family(const session_runtime::transport_protocol_e protocol) {
    return alk_gamestream_host_legacy_transport_protocol_family(
  ...
  alk_transport_candidate_type(const session_runtime::transport_route_e route) {
    switch (route) {
      ...
      case session_runtime::transport_route_e::overlay_route:
  ...
  alk_transport_flags(const session_runtime::transport_path_t &path) {
    ...
    switch (path.protocol) {
      case session_runtime::transport_protocol_e::provider_datagram:
  ...
  alk_transport_stack_flags(const session_runtime::transport_path_t &path) {
    return alk_gamestream_host_legacy_transport_stack_traits(
  ...
  send_control_datagram_via_transport_or_direct(session_t *session,
    ...
    alk_gamestream_host_enet_control_datagram_init(&datagram,
                                                       channel,
                                                       flags,
                                                       false,
                                                       payload.data(),
  ...
  record_network_transport_datagram(session_t *session, std::uint8_t channel, std::uint32_t flags, std::size_t length, bool tx) {
    ...
    alk_transport_runtime_record_datagram(&session->control.transport_binding,
                                          alk_gamestream_host_enet_control_channel_kind(channel, flags),
                                          length,
                                          tx);
  }
  ...
  open_network_transport_observer(session_t *session, net::peer_t peer) {
    ...
    alk_gamestream_host_enet_control_transport_open_params_init(&params,
  ...
  update_network_transport_peer_snapshot(session_t *session,
    ...
    AlkTransportStats stats;
    alk_transport_stats_init(&stats);
    if (!alk_transport_runtime_stats(&session->control.transport_binding, &stats)) {
      stats.state = control_transport_ready(session) ? ALK_TRANSPORT_PATH_ACTIVE : ALK_TRANSPORT_PATH_FAILED;
    ...
    alk_session_copy_string(stats.active_path.local_endpoint,
                            sizeof(stats.active_path.local_endpoint),
                            session->active_transport_path.host_local_endpoint.c_str());
    alk_transport_runtime_observe_stats(&session->control.transport_binding, &stats);
    alk_transport_monitor_observe_stats(&session->transport_monitor, &stats, steady_clock_ms());
    refresh_transport_monitor_views(session);
  }
  ...
  capture_control_peer_health(session_t *session,
    ...
    AlkTransportPeerStats peer_stats;
    alk_transport_peer_stats_init(&peer_stats);
    AlkTransportStats stats;
    alk_transport_stats_init(&stats);
    if (session->control.transport_peer_id != 0 &&
        alk_transport_runtime_peer_stats(&session->broadcast_ref->control_server._transport_binding,
                                         session->control.transport_peer_id,
                                         &peer_stats)) {
      snapshot.packet_loss_ppm = peer_stats.packet_loss_ppm;
      snapshot.waiting_queue_depth = peer_stats.reliable_backlog_bytes;
      snapshot.reliable_in_transit = 0;
      snapshot.send_reliable_queue_depth = peer_stats.reliable_backlog_packets;
      snapshot.sent_reliable_queue_depth = 0;
      stats.rtt_us = peer_stats.rtt_us;
      stats.jitter_us = peer_stats.jitter_us;
      stats.state = peer_stats.state;
    }
    else if (alk_transport_runtime_stats(&session->control.transport_binding, &stats)) {
      snapshot.packet_loss_ppm = stats.packet_loss_ppm;
      snapshot.waiting_queue_depth = stats.reliable_backlog_bytes;
      snapshot.reliable_in_transit = 0;
      snapshot.send_reliable_queue_depth = stats.reliable_backlog_packets;
      snapshot.sent_reliable_queue_depth = 0;
    }
    else {
      return false;
    ...
    health_sample.rtt_ms = stats.rtt_us / 1000U;
    health_sample.rtt_variance_ms = stats.jitter_us / 1000U;
  ...
  log_control_peer_diag(session_t *session, net::peer_t peer, const char *reason) {
    ...
    if (!capture_control_peer_health(session, nullptr, reason, health)) {
      BOOST_LOG(info) << "Control transport diag [" << reason << "] unavailable";
      return;
    }
    AlkTransportStats stats;
    alk_transport_stats_init(&stats);
    alk_transport_runtime_stats(&session->control.transport_binding, &stats);

    update_network_transport_peer_snapshot(session, health);
    ...
    BOOST_LOG(info) << "Control transport diag [" << reason << "] runtime=" << session->identity.runtime_id
                    << " state=" << stats.state
                    ...
                    << " rtt=" << (stats.rtt_us / 1000U) << "ms"
                    << " loss=" << (static_cast<double>(health.packet_loss_ppm) / 10000.0)
                    << "%"
                    ...
                    << health.waiting_queue_depth << "/"
                    << health.reliable_in_transit
  ...
  is_control_input_recent(void *channel_data) {
    ...
    const bool drag_active =
      session->drag_motion_active_until_ms.load(std::memory_order_relaxed) >= now_ms;
    const bool mouse_button_active =
      session->mouse_button_active_until_ms.load(std::memory_order_relaxed) >= now_ms;
    const bool gamepad_active =
      session->gamepad_input_active_until_ms.load(std::memory_order_relaxed) >= now_ms;
    const bool recent_control_input =
      session->last_control_input_received.time_since_epoch().count() != 0 &&
      std::chrono::steady_clock::now() - session->last_control_input_received <= 350ms;
    return stream_quality::input_activity_is_recent(recent_control_input,
                                                    drag_active,
                                                    mouse_button_active,
                                                    gamepad_active);
  }
  ...
  control_server_t::iterate(std::chrono::milliseconds timeout) {
    ...
    if (_transport_native_open) {
      for (int events_processed = 0; events_processed < max_events_per_iter; ++events_processed) {
        ...
        AlkGameStreamHostControlMessage control_message;
        if (!alk_gamestream_host_control_message_from_transport(&datagram, &control_message)) {
          continue;
        }
        std::string_view payload {
          reinterpret_cast<const char *>(control_message.payload),
          control_message.payload_length,
        };
        session->control.rx_events++;
        record_network_transport_datagram(session,
                                          static_cast<std::uint8_t>(control_message.carrier_channel_id),
                                          control_message.flags,
                                          control_message.payload_length + sizeof(control_message.wire_type),
                                          false);
        session->control.last_wire_type = control_message.wire_type;
        session->control.last_payload_size = payload.size();

        call(control_message.wire_type, session, payload, false);
        alk_transport_runtime_service(&_transport_binding, nullptr);
      }
      return;
    }
    ...
    BOOST_LOG(error) << "Legacy GameStream control carrier event loop is disabled; network.transport/gamestream-enet native IO must be open";
  }


  // GameStream RTP/FEC implementation moved to Alkaid media.packetization / gamestream-rtp-fec.
  ...
  cursor_plane_sample_from_cursor_state(session_t *session, const AlkCursorState &state) {
    ...
    if (fields.bitmap_pixels != nullptr && fields.bitmap_bytes != 0) {
  ...
  ensure_alkaid_host_cursor_initialized(session_t *session) {
    ...
    if (!alk_interaction_cursor_backend_provider_init_native_win32_host_capture(
      ...
      BOOST_LOG(warning) << "interaction.cursor native host Backend Provider unavailable";
      return false;
    }
  ...
  send_cursor_plane_update(session_t *session,
    ...
    cursor->flags = util::endian::little<std::uint32_t>(
      include_bitmap ? (flags | SS_CURSOR_PLANE_FLAG_SHAPE_BITMAP) :
                       (flags & ~SS_CURSOR_PLANE_FLAG_SHAPE_BITMAP));
    const bool first_legacy_cursor_packet = session->control.cursor_plane.epoch == 0;
  ...
  maybe_send_cursor_plane_update(session_t *session, std::chrono::steady_clock::time_point now) {
    ...
    const bool geometry_changed =
      (emit_plan.reason_flags & ALK_CURSOR_HOST_EMIT_REASON_GEOMETRY_CHANGED) != 0u;
    const bool flags_changed =
      (emit_plan.reason_flags & ALK_CURSOR_HOST_EMIT_REASON_FLAGS_CHANGED) != 0u;
    ...
    if (session_cursor_sent || legacy_sent) {
      ...
      if (never_sent || shape_changed || should_send_bitmap || session_cursor_needs_initial_replay) {
        BOOST_LOG(info) << "Cursor plane update sent runtime=" << session->identity.runtime_id
                        ...
                        << " sourceDim=" << sample.source_width << "x" << sample.source_height
                        << " bitmap=" << (should_send_bitmap ? sample.bitmap_bgra.size() : 0)
                        << " bitmapHash=0x" << util::hex(sample.bitmap_hash).to_string_view()
                        << " assetEpoch=" << sample.animation_epoch
                        << " interaction.cursor=platform"
                        << " emitReason=0x" << util::hex(emit_plan.reason_flags).to_string_view();
      }
    }
  }
  ...
  send_session_control_payload(session_t *session,
                               ...
                               std::uint32_t packet_flags = ALK_TRANSPORT_SEND_FLAG_RELIABLE) {
  ...
  send_rescue_control_ack(session_t *session,
                          const AlkGameStreamHostRescueWireAck &ack,
                          std::uint32_t packet_flags = ALK_TRANSPORT_SEND_FLAG_RELIABLE) {
    if (!session || !control_transport_ready(session)) {
    ...
    ALK_GAMESTREAM_HOST_RESCUE_ACK ack_payload;
    if (!alk_gamestream_host_rescue_build_ack(&ack, &ack_payload)) {
  ...
  static bool
  rescue_request_is_media_only(const AlkRescueControlRequest &request) {
    const auto non_media_triggers =
      ALK_RESCUE_TRIGGER_CONTROL_STALLING |
      ALK_RESCUE_TRIGGER_MANUAL;
    const auto media_triggers =
      ALK_RESCUE_TRIGGER_RFI_WAIT |
      ALK_RESCUE_TRIGGER_UNRECOVERABLE_FRAMES |
      ALK_RESCUE_TRIGGER_LOCAL_DISPLAY_STALL;
    return (request.trigger_flags & non_media_triggers) == 0 &&
           (request.trigger_flags & media_triggers) != 0;
  }
  ...
  bool
  rescue_transport_stats_clean_enough_for_verified_capacity(std::uint32_t rtt_us,
                                                            std::uint32_t jitter_us,
                                                            std::uint32_t packet_loss_ppm,
                                                            std::uint32_t reliable_backlog_packets) {
    return stream_quality::verified_capacity_rescue_transport_clean(
      rtt_us,
      jitter_us,
      packet_loss_ppm,
      reliable_backlog_packets);
  }

  bool
  rescue_transport_path_clean_enough_for_verified_capacity(const session_runtime::transport_path_t &path) {
    return path.score.rtt_ms <= 80U &&
           path.score.jitter_ms <= 18U &&
           path.score.loss_ppm <= 3000U;
  }

  static bool
  verified_capacity_transport_clean_for_rescue(session_t *session,
                                               AlkTransportStats *snapshot = nullptr) {
    if (!session) {
      return false;
    }

    if (session->last_stream_quality_link_quality_state !=
        stream_quality::link_quality_state_e::verified_good) {
      return false;
    }

    AlkTransportStats stats;
    alk_transport_stats_init(&stats);
    const bool have_stats = snapshot_transport_stats(session, stats);
    if (snapshot) {
      *snapshot = stats;
    }
    if (have_stats) {
      return rescue_transport_stats_clean_enough_for_verified_capacity(
        stats.rtt_us,
        stats.jitter_us,
        stats.packet_loss_ppm,
        stats.reliable_backlog_packets);
    }

    const auto &path = session->active_transport_path;
    return rescue_transport_path_clean_enough_for_verified_capacity(path);
  }
  ...
  apply_rescue_control_request(session_t *session, const AlkRescueControlRequest &request) {
    ...
    AlkTransportStats rescue_transport_stats;
    alk_transport_stats_init(&rescue_transport_stats);
    const bool media_only_rescue = rescue_request_is_media_only(request);
    const bool rescue_requests_quality_downgrade =
      stream_quality::rescue_request_requests_quality_downgrade(
        request.trigger_flags,
        request.max_bitrate_kbps,
        request.fec_percent,
        request.target_scale_percent,
        request.enable_blurry_upscale);
    const bool media_quality_downgrade_allowed =
      !media_only_rescue ||
      stream_quality::media_rescue_quality_downgrade_allowed(
        request.trigger_flags,
        rescue_requests_quality_downgrade,
        session->stream_quality_diag.missing_packets,
        session->stream_quality_diag.total_packets,
        session->stream_quality_diag.unrecoverable_frames,
        session->stream_quality_diag.waiting_for_rfi_frames,
        session->stream_quality_diag.large_frame_fec_skipped,
        session->active_transport_path.score.loss_ppm);
    const bool media_quality_gate =
      media_only_rescue && rescue_requests_quality_downgrade && !media_quality_downgrade_allowed;
    const bool verified_capacity_clean_rescue =
      media_only_rescue &&
      verified_capacity_transport_clean_for_rescue(session, &rescue_transport_stats);
    if (media_only_rescue && (!rescue_requests_quality_downgrade || media_quality_gate)) {
      const bool repeated_media_rescue =
        coalesced >= 2 ||
        session->rescue_control_requests >= 3 ||
        request.hold_ms >= 1000 ||
        (request.trigger_flags & ALK_RESCUE_TRIGGER_UNRECOVERABLE_FRAMES) != 0;
      const bool degraded_media_rescue =
        !media_quality_gate &&
        !verified_capacity_clean_rescue &&
        ((request.trigger_flags & ALK_RESCUE_TRIGGER_RFI_WAIT) != 0 ||
         (request.trigger_flags & ALK_RESCUE_TRIGGER_LOCAL_DISPLAY_STALL) != 0 ||
         rescue_transport_stats.packet_loss_ppm >= 30000 ||
         rescue_transport_stats.jitter_us >= 18000 ||
         rescue_transport_stats.reliable_backlog_packets >= 8);
      const bool extended_media_resync =
        degraded_media_rescue &&
        repeated_media_rescue &&
        !rescue_control_media_only_request_allows_runtime_fps_drain();
      if (extended_media_resync) {
        ...
        session->stream_quality_resync_guard_until = std::max(session->stream_quality_resync_guard_until,
                                                              now + std::chrono::milliseconds(hold_ms));
        if (request.request_idr) {
          session->video.idr_events->raise(true);
        }
        BOOST_LOG(info) << "Alkaid rescue-control media recovery resync"
                        << " runtime=" << session->identity.runtime_id
                        << " requestId=" << request.request_id
                        << " triggers=0x" << std::hex << request.trigger_flags << std::dec
                        << " action=idr-only-extended-hold"
                        << " bitrate=" << session->last_applied_stream_quality_bitrate << " Kbps"
                        << " fec=" << session->last_applied_stream_quality_fec << "%"
                        << " fps=" << session->last_applied_stream_quality_fps
                        << " holdMs=" << hold_ms
                        << " coalesced=" << coalesced
                        << " rtt=" << (rescue_transport_stats.rtt_us / 1000U) << "ms"
                        << " jitter=" << (rescue_transport_stats.jitter_us / 1000U) << "ms"
                        << " transportLossPpm=" << rescue_transport_stats.packet_loss_ppm
                        << " backlogPackets=" << rescue_transport_stats.reliable_backlog_packets;
        return true;
      }
      const auto hold_ms = std::clamp<std::uint32_t>(request.hold_ms == 0 ? 350u : request.hold_ms,
                                                     300u,
                                                     1000u);
      session->rescue_control_hold_until = now + std::chrono::milliseconds(hold_ms);
      if (request.request_idr) {
      ...
      BOOST_LOG(info) << "Alkaid rescue-control media recovery idr-only"
                      << " runtime=" << session->identity.runtime_id
                      << " requestId=" << request.request_id
                      << " triggers=0x" << std::hex << request.trigger_flags << std::dec
                      << " action=idr-only"
                      ...
                      << " holdMs=" << hold_ms
                      << " coalesced=" << coalesced
                      << " qualityGate=" << (media_quality_gate ? 1 : 0)
                      << " hostVideoPackets=" << session->stream_quality_diag.missing_packets
                      << "/" << session->stream_quality_diag.total_packets
                      << " hostUnrecoverable=" << session->stream_quality_diag.unrecoverable_frames
                      << " hostWaitingForRfi=" << session->stream_quality_diag.waiting_for_rfi_frames;
      return true;
    }
    if (verified_capacity_clean_rescue) {
      session->rescue_control_hold_until = now + 350ms;
      if (request.request_idr) {
        session->video.idr_events->raise(true);
      }
      BOOST_LOG(info) << "Alkaid rescue-control ignored weak-network downgrade on clean verified path"
                      << " runtime=" << session->identity.runtime_id
                      << " requestId=" << request.request_id
                      << " triggers=0x" << std::hex << request.trigger_flags << std::dec
                      << " action=idr-only"
    ...
    int target_scale = 100;
    if (request.target_scale_percent > 0 && request.target_scale_percent < 100) {
      target_scale = static_cast<int>(request.target_scale_percent);
    }
    if (session->last_applied_stream_quality_resolution_scale > 0 &&
        session->last_applied_stream_quality_resolution_scale < target_scale) {
      target_scale = session->last_applied_stream_quality_resolution_scale;
    }
  ...
  maybe_apply_control_path_health_rescue(session_t *session,
    ...
    const bool accepted = apply_rescue_control_request(session, rescue_decision.request);
      ...
      alk_transport_stats_init(&transport_stats);
      alk_transport_runtime_stats(&session->control.transport_binding, &transport_stats);
	      session->last_control_path_rescue_log = now;
	      BOOST_LOG(info) << "Alkaid rescue-control local health rescue applied"
                      << " runtime=" << session->identity.runtime_id
                      << " health=" << alk_control_path_health_state_name(health.decision.state)
                      << " healthReason=0x" << std::hex << health.decision.reason_flags << std::dec
                      << " rtt=" << (transport_stats.rtt_us / 1000U) << "ms"
                      << " lastRecvAge=" << health.last_recv_age << "ms"
                      << " nextTimeoutIn=" << health.next_timeout_in << "ms"
                      << " bitrate=" << session->last_applied_stream_quality_bitrate << " Kbps"
                      << " fec=" << session->last_applied_stream_quality_fec << "%"
  ...
  send_session_control_cursor_plane(session_t *session, const cursor_plane_sample_t &sample) {
    ...
    AlkCursorState alkaid_cursor_state {};
    alk_cursor_state_init(&alkaid_cursor_state);
    alkaid_cursor_state.seq_known = ALK_INTERACTION_CURSOR_SEQ_UNKNOWN;
    alkaid_cursor_state.visible = (sample.flags & SS_CURSOR_PLANE_FLAG_VISIBLE) != 0 ? 1u : 0u;
    alkaid_cursor_state.locked = (sample.flags & SS_CURSOR_PLANE_FLAG_LOCKED) != 0 ? 1u : 0u;
    alkaid_cursor_state.pointer_input_mode =
      (sample.flags & SS_CURSOR_PLANE_FLAG_RELATIVE) != 0 ?
        ALK_POINTER_INPUT_MODE_RELATIVE :
        ALK_POINTER_INPUT_MODE_ABSOLUTE;
    alkaid_cursor_state.pointer_lock_state = alkaid_cursor_state.locked != 0u ?
                                              ALK_POINTER_LOCK_STATE_LOCKED :
                                              ALK_POINTER_LOCK_STATE_FREE;
    alkaid_cursor_state.asset.asset_id = sample.cursor_shape_id;
    alkaid_cursor_state.asset.format =
      sample.bitmap_bgra.empty() ? ALK_CURSOR_ASSET_FORMAT_SYSTEM_CURSOR_ID :
                                   ALK_CURSOR_ASSET_FORMAT_BGRA_PREMULTIPLIED;
    alkaid_cursor_state.asset.blend_mode = ALK_CURSOR_BLEND_MODE_ALPHA;
    alkaid_cursor_state.asset.mask_semantics = ALK_CURSOR_MASK_SEMANTICS_ALPHA;
    alkaid_cursor_state.asset.width = sample.display_width;
    alkaid_cursor_state.asset.height = sample.display_height;
    alkaid_cursor_state.asset.hotspot_x = sample.display_hotspot_x;
    ...
    const bool alkaid_asset_semantics =
      alk_cursor_asset_has_render_semantics(&alkaid_cursor_state.asset);
    ...
    if (result == 0) {
      BOOST_LOG(debug) << "Session cursor plane sent runtime="sv
                       ...
                       << " inputMode="sv << alkaid_cursor_state.pointer_input_mode
                       << " lockState="sv << alkaid_cursor_state.pointer_lock_state;
    }
    return result;
  }
  ...
  static void
  reset_clipboard_backend_staging(session_t *session) {
    if (!session) {
      return;
    }
    session->control.clipboard.last_host_sequence = 0;
    session->control.clipboard.staged_item_data.clear();
    session->control.clipboard.staged_mime_type.clear();
    session->control.clipboard.staged_name.clear();
    session->control.clipboard.last_read_status = clipboard_read_status_e::none;
  }

  static bool
  ensure_clipboard_sync(session_t *session) {
#ifdef _WIN32
    if (!session || !config::input.clipboard_sync || !platf::clipboard::is_backend_available()) {
      return false;
    }

    if (session->control.clipboard.sync) {
      return true;
    }

    const auto backend_caps = platf::clipboard::supported_capabilities();
    if (backend_caps == 0) {
      return false;
    }

    auto sync = clipboard_sync_ptr { alk_clipboard_sync_create(), &alk_clipboard_sync_destroy };
    if (!sync) {
      return false;
    }

    AlkClipboardSyncConfig clipboard_config;
    alk_clipboard_sync_config_init(&clipboard_config);
    clipboard_config.role = ALK_SESSION_ROLE_HOST;
    clipboard_config.capability_flags = ALK_CLIPBOARD_SYNC_CAP_TEXT_UTF8 |
                                        ALK_CLIPBOARD_SYNC_CAP_PNG_IMAGE |
                                        ALK_CLIPBOARD_SYNC_CAP_CHUNKED_ITEMS |
                                        ALK_CLIPBOARD_SYNC_CAP_ECHO_SUPPRESSION;
    clipboard_config.max_chunk_size = LI_CLIPBOARD_MAX_CHUNK_SIZE;
    clipboard_config.max_text_size = 1U * 1024U * 1024U;
    clipboard_config.max_image_size = static_cast<std::uint32_t>(platf::clipboard::image_size_limit);
    clipboard_config.max_blob_size = 0;
    clipboard_config.blob_inline_threshold = 0;
    clipboard_config.backend_mode = ALK_CLIPBOARD_SYNC_BACKEND_DIRECT_PLATFORM;

    AlkClipboardSyncCallbacks callbacks;
    alk_clipboard_sync_callbacks_init(&callbacks);
    callbacks.user_data = session;
    callbacks.send_module_message = [](void *user_data,
                                       const AlkModuleMessage *message,
                                       const void *payload,
      ...
      if (!session || payload == nullptr || payload_length == 0) {
        return false;
      }
      return send_clipboard_payload(session,
                                    std::string_view {
                                      reinterpret_cast<const char *>(payload),
                                      payload_length,
                                    }) == 0;
    };
    callbacks.read_local_clipboard = [](void *user_data, AlkClipboardSyncItem *item) {
      auto *session = static_cast<session_t *>(user_data);
      if (!session || item == nullptr) {
        return false;
      }

      alk_clipboard_sync_item_init(item);

#ifdef _WIN32
      platf::clipboard::item_t local_item;
      std::string reason;
      if (!platf::clipboard::read_current_item(local_item, &reason)) {
        session->control.clipboard.last_read_status = clipboard_read_status_e::failed;
        if (!reason.empty()) {
      ...
      if (local_item.type == LI_CLIPBOARD_ITEM_TYPE_NONE) {
        session->control.clipboard.staged_item_data.clear();
        session->control.clipboard.staged_mime_type.clear();
        session->control.clipboard.staged_name.clear();
        session->control.clipboard.last_read_status = clipboard_read_status_e::skipped;
        if (!reason.empty()) {
      ...
      if (local_item.type != LI_CLIPBOARD_ITEM_TYPE_TEXT &&
          local_item.type != LI_CLIPBOARD_ITEM_TYPE_IMAGE) {
        return false;
      }

      session->control.clipboard.staged_item_data = std::move(local_item.data);
      session->control.clipboard.staged_mime_type = std::move(local_item.mime_type);
      session->control.clipboard.staged_name = std::move(local_item.name);
      session->control.clipboard.last_read_status = clipboard_read_status_e::item;
      item->type = local_item.type == LI_CLIPBOARD_ITEM_TYPE_TEXT ?
                     ALK_CLIPBOARD_SYNC_ITEM_TEXT_UTF8 :
                     ALK_CLIPBOARD_SYNC_ITEM_PNG;
      item->flags = 0;
      item->data = session->control.clipboard.staged_item_data.empty() ?
                     nullptr :
                     session->control.clipboard.staged_item_data.data();
      item->length = session->control.clipboard.staged_item_data.size();
      item->mime_type = session->control.clipboard.staged_mime_type.c_str();
      item->name = session->control.clipboard.staged_name.c_str();
      item->content_hash = local_item.content_hash;
      return true;
#else
      (void) session;
      return false;
    ...
    callbacks.write_local_clipboard = [](void *user_data, const AlkClipboardSyncItem *item) {
      auto *session = static_cast<session_t *>(user_data);
      if (!session || item == nullptr || item->version != ALK_CLIPBOARD_SYNC_VERSION) {
        return false;
      }

#ifdef _WIN32
      platf::clipboard::item_t local_item;
      local_item.type = item->type == ALK_CLIPBOARD_SYNC_ITEM_TEXT_UTF8 ?
                          LI_CLIPBOARD_ITEM_TYPE_TEXT :
                        item->type == ALK_CLIPBOARD_SYNC_ITEM_PNG ?
                          LI_CLIPBOARD_ITEM_TYPE_IMAGE :
                          LI_CLIPBOARD_ITEM_TYPE_NONE;
      local_item.content_hash = item->content_hash;
      if (item->length != 0 && item->data != nullptr) {
        local_item.data.assign(item->data, item->data + item->length);
      }
      if (item->mime_type != nullptr) {
        local_item.mime_type = item->mime_type;
      }
      ...
      std::string reason;
      if (!platf::clipboard::write_item(local_item, &reason)) {
        if (!reason.empty()) {
    ...
    callbacks.now_ms = [](void *) {
      return static_cast<std::uint64_t>(
        std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::steady_clock::now().time_since_epoch()).count());
    };

    if (!alk_clipboard_sync_init(sync.get(), &clipboard_config, &callbacks)) {
      return false;
    }

    session->control.clipboard.sync = std::move(sync);
    reset_clipboard_backend_staging(session);
    BOOST_LOG(info) << "Clipboard module backend initialized runtime=" << session->identity.runtime_id
                    << " provider=" << kSunshineClipboardBackendProviderId
                    << " slot=" << ALK_CLIPBOARD_SYNC_SLOT_ID
                    << " module=" << ALK_CLIPBOARD_SYNC_MODULE_ID
                    << " contract=" << ALK_CLIPBOARD_SYNC_CONTRACT_ID;
    return true;
#else
    (void) session;
    return false;
  ...
  static bool
  clipboard_is_owner(session_t *session) {
    return session != nullptr &&
           session->control.clipboard.sync != nullptr &&
           alk_clipboard_sync_is_owner_bound(session->control.clipboard.sync.get()) &&
           session->broadcast_ref &&
           session->broadcast_ref->feature_leases.validate(session_runtime::feature_e::clipboard, *session);
  }

  static void
  clipboard_release_owner(session_t *session) {
    if (!session) {
      return;
    }
    session->control.clipboard.last_host_sequence = 0;
  ...
  static void
  clipboard_reset_inbound_state(session_t *session) {
    reset_clipboard_backend_staging(session);
  }

  [[maybe_unused]] static int
  clipboard_request_snapshot(session_t *session) {
    if (!ensure_clipboard_sync(session) || session->control.clipboard.sync == nullptr) {
      return send_empty_clipboard_snapshot(session);
    }
    return alk_clipboard_sync_request_snapshot(session->control.clipboard.sync.get()) ? 0 : -1;
  }

  static int
  clipboard_notify_local_change(session_t *session) {
#ifdef _WIN32
    if (!ensure_clipboard_sync(session) || session->control.clipboard.sync == nullptr) {
      return -1;
    }
    ...
    if (sequence == 0 || sequence == session->control.clipboard.last_host_sequence) {
      return 0;
    }

    const bool changed = alk_clipboard_sync_notify_local_change(session->control.clipboard.sync.get());
    if (changed) {
      session->control.clipboard.last_host_sequence = sequence;
      return 1;
    }
    if (session->control.clipboard.last_read_status == clipboard_read_status_e::skipped) {
      session->control.clipboard.last_host_sequence = sequence;
      return 0;
    }
    return -1;
  ...
  controlBroadcastThread(control_server_t *server) {
    auto is_clipboard_owner = [](session_t *session) {
      return clipboard_is_owner(session);
    };
    ...
    auto maybe_send_host_clipboard_update = [client_supports_clipboard, is_clipboard_owner](session_t *session) {
      if (!client_supports_clipboard(session) || !is_clipboard_owner(session)) {
        return;
      }
      if (clipboard_notify_local_change(session) < 0) {
    ...
    constexpr std::uint32_t control_mic_magic = ALK_GAMESTREAM_HOST_MICROPHONE_CONTROL_DATA_MAGIC;
    constexpr std::uint16_t control_mic_version = ALK_GAMESTREAM_HOST_MICROPHONE_CONTROL_VERSION;
    constexpr std::uint16_t control_mic_flag_session_token = ALK_GAMESTREAM_HOST_MICROPHONE_CONTROL_FLAG_SESSION_TOKEN;
    constexpr std::size_t control_mic_token_size = ALK_GAMESTREAM_HOST_MICROPHONE_SESSION_TOKEN_LENGTH;
    ...
    server->map(packetTypes[IDX_MIC_DATA], [&](session_t *session, const std::string_view &payload) {
      ...
      AlkGameStreamHostMicrophoneControlPacket mic_packet;
      if (!alk_gamestream_host_microphone_parse_control_packet(payload.data(), payload.size(), &mic_packet) ||
    ...
    server->map(packetTypes[IDX_CLIPBOARD], [&, is_clipboard_owner, client_supports_clipboard, client_supports_clipboard_item](session_t *session, const std::string_view &payload) {
      ...
      if (!config::input.clipboard_sync || !platf::clipboard::is_backend_available()) {
        ...
        clipboard_release_owner(session);
      ...
      const auto kind = static_cast<std::uint8_t>(payload.front());
      if (!ensure_clipboard_sync(session)) {
        BOOST_LOG(warning) << "Clipboard module backend unavailable for client " << session->client_name;
        return;
      }

      auto *sync = session->control.clipboard.sync.get();
      switch (kind) {
        case ALK_CLIPBOARD_SYNC_MSG_BIND: {
          ...
          for (auto *other: *server->_sessions) {
            if (other != session) {
              if (other->broadcast_ref) {
                other->broadcast_ref->feature_leases.release(session_runtime::feature_e::clipboard, *other);
              }
              clipboard_release_owner(other);
            }
          }
          session->broadcast_ref->feature_leases.acquire(session_runtime::feature_e::clipboard, *session);
          clipboard_reset_inbound_state(session);
          if (!alk_clipboard_sync_bind_owner(sync)) {
            BOOST_LOG(warning) << "Failed to bind clipboard module owner for client " << session->client_name;
            session->broadcast_ref->feature_leases.release(session_runtime::feature_e::clipboard, *session);
            break;
          }
        ...
        case ALK_CLIPBOARD_SYNC_MSG_UNBIND:
          session->broadcast_ref->feature_leases.release(session_runtime::feature_e::clipboard, *session);
          clipboard_release_owner(session);
          BOOST_LOG(info) << "Clipboard session unbound for client " << session->client_name;
          break;
        case ALK_CLIPBOARD_SYNC_MSG_SNAPSHOT_REQUEST:
          ...
          if (!alk_clipboard_sync_request_snapshot(sync)) {
            BOOST_LOG(warning) << "Failed to send empty clipboard snapshot to client " << session->client_name;
          }
          break;
        case ALK_CLIPBOARD_SYNC_MSG_ITEM_START: {
          if (payload.size() < 28u) {
            BOOST_LOG(warning) << "Clipboard ITEM_START payload too short";
            break;
          }
          const auto item_type = static_cast<std::uint8_t>(payload[2]);
          if (!is_clipboard_owner(session)) {
          ...
          if (!alk_clipboard_sync_on_remote_message(sync, payload.data(), payload.size())) {
            BOOST_LOG(warning) << "Failed to apply clipboard item start for client " << session->client_name;
          }
          break;
        }
        case ALK_CLIPBOARD_SYNC_MSG_ITEM_CHUNK:
        case ALK_CLIPBOARD_SYNC_MSG_ITEM_END:
        case ALK_CLIPBOARD_SYNC_MSG_ITEM_CANCEL: {
          if (!is_clipboard_owner(session)) {
            BOOST_LOG(warning) << "Ignoring clipboard item payload from non-owner client " << session->client_name;
            return;
          }
          if (!alk_clipboard_sync_on_remote_message(sync, payload.data(), payload.size())) {
            BOOST_LOG(warning) << "Failed to apply clipboard item payload for client " << session->client_name;
          }
          break;
        }
        default:
    ...
    server->map(packetTypes[IDX_RESCUE], [&](session_t *session, const std::string_view &payload) {
      ...
      if (!alk_gamestream_host_rescue_parse_request(payload.data(), payload.size(), &request)) {
      ...
      AlkGameStreamHostRescueWireAck ack;
      alk_gamestream_host_rescue_ack_init(&ack);
      ...
      ack.applied_fec_percent = session->last_applied_stream_quality_fec >= 0 ?
                                  ...
                                  0u;
      if (send_rescue_control_ack(session, ack, ALK_TRANSPORT_SEND_FLAG_UNSEQUENCED) == 0) {
    ...
    server->map(SS_FRAME_FEC_PTYPE, [&](session_t *session, const std::string_view &payload) {
      ...
      if (payload.size() < sizeof(AlkGamestreamFrameFecStatus)) {
      ...
      const auto *status = reinterpret_cast<const AlkGamestreamFrameFecStatus *>(payload.data());
      ...
      process_stream_quality_feedback(session, feedback, "fec");
    });
    ...
    server->map(SS_NETWORK_FEEDBACK_PTYPE, [&](session_t *session, const std::string_view &payload) {
      ...
      process_stream_quality_feedback(session, network_feedback, "feedback");
      const auto now = std::chrono::steady_clock::now();
    ...
    server->map(packetTypes[IDX_REQUEST_IDR_FRAME], [&](session_t *session, const std::string_view &payload) {
      BOOST_LOG(info) << "Control IDR request received"sv
                      << " runtime="sv << (session ? session->identity.runtime_id : 0)
                      << " payloadBytes="sv << payload.size()
                      << " decrypted="sv << (session ? session->control.decrypted_rx_events : 0)
                      << " rx="sv << (session ? session->control.rx_events : 0);
      ...
      if (!should_forward_client_recovery_request(session,
                                                  ...
                                                  450ms)) {
        BOOST_LOG(info) << "Control IDR request coalesced"sv
                        << " runtime="sv << session->identity.runtime_id;
        return;
      }
      report_stream_quality_recovery_request(session, "idr");
      session->video.idr_events->raise(true);
      BOOST_LOG(info) << "Control IDR event raised"sv
                      << " runtime="sv << session->identity.runtime_id;
    });
    ...
    server->map(packetTypes[IDX_ENCRYPTED], [server](session_t *session, const std::string_view &payload) {
      ...
      if (type == packetTypes[IDX_REQUEST_IDR_FRAME] || type == packetTypes[IDX_RESCUE]) {
        BOOST_LOG(info) << "Control recovery packet decrypted"sv
                        << " runtime="sv << session->identity.runtime_id
                        << " seq="sv << seq
                        << " type="sv << util::hex(type).to_string_view()
                        << " payloadBytes="sv << next_payload.size()
                        << " channelEncryptedRx="sv << session->control.encrypted_rx_events
                        << " decrypted="sv << session->control.decrypted_rx_events;
      }
...

      {
        ...
        KITTY_WHILE_LOOP(auto pos = std::begin(*server->_sessions), pos != std::end(*server->_sessions), {
          ...
          if (now > session->pingTimeout) {
            auto address = control_peer_label(session);
          ...
          if (!control_transport_ready(session)) {
            ...
            // Cursor-plane legacy compatibility must not depend on Session Control
            // welcome. Older/Qt clients can advertise ML_FF2_CURSOR_PLANE and
            // preferCursorPlane without supporting the Session Control channel.
            // If we gate this behind session_control_welcome_sent, Sunshine disables
            // video cursor burn-in for the stream but never sends SS_CURSOR_PLANE,
            // leaving those clients with only their local OS cursor visible.
  ...
  micRecvThread(broadcast_ctx_t &ctx) {
    ...
    mic_recv_func = [&](const boost::system::error_code &ec, size_t received_bytes) {
      ...
      if (received_bytes < sizeof(AlkGamestreamRtpPacket)) {
        return;
      }
  ...
  recvThread(broadcast_ctx_t &ctx) {
    ...
    // 初始化接收函数
    auto init_recv_func = [&](auto &sock, size_t buf_idx, auto &session_map, std::string_view type_str, std::uint32_t channel_kind) {
      recv_funcs[buf_idx] = [&, buf_idx, type_str, channel_kind](const boost::system::error_code &ec, size_t bytes) {
        // 静默处理正常关闭错误
        if (ec == boost::asio::error::operation_aborted ||
            ec == boost::asio::error::bad_descriptor) {
        ...
        // 只有在成功接收数据后才重新调度
        try {
          alk_gamestream_host_media_udp_async_receive_from(channel_kind,
                                                               sock,
                                                               asio::buffer(buffers[buf_idx]),
                                                               peer,
                                                               recv_funcs[buf_idx]);
        }
        catch (const std::exception &e) {
    ...
    try {
      init_recv_func(video_sock, 0, peer_to_video_session, "VIDEO", ALK_TRANSPORT_CHANNEL_VIDEO_DATAGRAM);
      init_recv_func(audio_sock, 1, peer_to_audio_session, "AUDIO", ALK_TRANSPORT_CHANNEL_AUDIO_DATAGRAM);

      alk_gamestream_host_media_udp_async_receive_from(ALK_TRANSPORT_CHANNEL_VIDEO_DATAGRAM,
                                                           video_sock,
                                                           asio::buffer(buffers[0]),
                                                           peer,
                                                           recv_funcs[0]);
      alk_gamestream_host_media_udp_async_receive_from(ALK_TRANSPORT_CHANNEL_AUDIO_DATAGRAM,
                                                           audio_sock,
                                                           asio::buffer(buffers[1]),
                                                           peer,
                                                           recv_funcs[1]);
    ...
    while (auto packet = packets->pop()) {
      ...
      frame_header.frameType = packet->is_idr()                     ? 2 :
                               packet->after_ref_frame_invalidation ? 5 :
                                                                      1;
      const auto video_payload_block_bytes = alk_gamestream_video_payload_block_bytes(session->config.packetsize);
      if (video_payload_block_bytes == 0) {
        BOOST_LOG(error) << "Invalid GameStream video packet size for RTP payload layout: "sv << session->config.packetsize;
        continue;
      }
      frame_header.lastPayloadLen = (payload.size() + sizeof(frame_header)) % video_payload_block_bytes;
      if (frame_header.lastPayloadLen == 0) {
        frame_header.lastPayloadLen = video_payload_block_bytes;
      }
      ...
      AlkGamestreamVideoPayloadLayout payload_layout;
      if (!alk_gamestream_video_payload_layout_build(payload.size(),
                                                     sizeof(frame_header),
                                                     session->config.packetsize,
                                                     &payload_layout)) {
        BOOST_LOG(error) << "Alkaid GameStream video RTP payload layout failed"sv;
        continue;
      }

      std::vector<uint8_t> payload_new(payload_layout.wire_payload_bytes);
      size_t payload_new_bytes = 0;
      if (!alk_gamestream_video_payload_build(payload_new.data(),
                                              payload_new.size(),
                                              &payload_new_bytes,
                                              &frame_header,
                                              sizeof(frame_header),
                                              ...
                                              payload.size(),
                                              session->config.packetsize)) {
        BOOST_LOG(error) << "Alkaid GameStream video RTP payload build failed"sv;
        continue;
      }

      payload = std::string_view { (char *) payload_new.data(), payload_new_bytes };

      AlkGamestreamVideoFrameWirePlan frame_wire_plan;
      if (!alk_gamestream_video_frame_wire_plan_build(payload.size(),
                                                      session->config.packetsize,
                                                      static_cast<std::uint32_t>(fecPercentage),
                                                      static_cast<std::uint32_t>(session->config.minRequiredFecPackets),
                                                      &frame_wire_plan)) {
        BOOST_LOG(error) << "Alkaid GameStream video RTP/FEC frame plan failed"sv;
        continue;
      }
      if (frame_wire_plan.large_frame_fec_skipped) {
        BOOST_LOG(warning) << "Skipping FEC for abnormally large encoded frame (needed more than "sv
                           << ALK_GAMESTREAM_VIDEO_MAX_FEC_BLOCKS << " FEC blocks)"sv;
        session->stream_quality_large_frame_fec_skipped.fetch_add(1, std::memory_order_relaxed);
      }
      fecPercentage = static_cast<int>(frame_wire_plan.fec_percentage);
      const auto blocksize = static_cast<size_t>(frame_wire_plan.block_bytes);
      const auto fec_blocks_needed = static_cast<size_t>(frame_wire_plan.fec_block_count);

      std::array<std::string_view, ALK_GAMESTREAM_VIDEO_MAX_FEC_BLOCKS> fec_blocks;
      ...
      if (frame_wire_plan.aligned_fec_block_bytes / blocksize >= 1024) {
        BOOST_LOG(error) << "Encoder produced a frame too large to send! Is the encoder broken? (needed "sv
                         << (frame_wire_plan.aligned_fec_block_bytes / blocksize) << " packets)"sv;
      }

      for (size_t x = 0; x < fec_blocks_needed; ++x) {
        fec_blocks[x] = payload.substr(frame_wire_plan.block_offsets[x], frame_wire_plan.block_lengths[x]);
      }
      ...
      try {
        ...
        std::for_each(fec_blocks_begin, fec_blocks_end, [&](std::string_view &current_payload) {
          if (!alk_gamestream_video_prepare_data_shard_headers(const_cast<char *>(current_payload.data()),
                                                               current_payload.size(),
                                                               static_cast<std::uint32_t>(blocksize),
                                                               packet->frame_index(),
                                                               static_cast<std::uint32_t>(lowseq),
                                                               static_cast<std::uint32_t>(blockIndex),
                                                               static_cast<std::uint32_t>(fec_blocks_needed))) {
            BOOST_LOG(error) << "Alkaid GameStream video RTP pre-FEC header prepare failed"sv;
            return;
          }

          frame_fec_latency_logger.first_point_now();
          std::unique_ptr<AlkGamestreamVideoFecFrame, decltype(&alk_gamestream_video_fec_frame_destroy)> shards(
            alk_gamestream_video_fec_frame_create(),
            alk_gamestream_video_fec_frame_destroy);
          if (!shards || !alk_gamestream_video_fec_encode(shards.get(),
            ...
            BOOST_LOG(error) << "Alkaid GameStream video RTP/FEC encode failed"sv;
            return;
          }
          frame_fec_latency_logger.second_point_now_and_log();
          const auto shard_count = alk_gamestream_video_fec_frame_shard_count(shards.get());
          const auto data_shards = alk_gamestream_video_fec_frame_data_shards(shards.get());
          const auto shard_block_bytes = alk_gamestream_video_fec_frame_block_bytes(shards.get());
          const auto shard_prefix_bytes = alk_gamestream_video_fec_frame_prefix_bytes(shards.get());
          const auto shard_percentage = alk_gamestream_video_fec_frame_percentage(shards.get());
          frame_sent_shards += shard_count;
          frame_sent_bytes += static_cast<std::uint64_t>(shard_count) *
                              static_cast<std::uint64_t>(shard_block_bytes + shard_prefix_bytes);

          std::vector<platf::buffer_descriptor_t> payload_buffers;
          const auto payload_buffer_count = alk_gamestream_video_fec_frame_payload_buffer_count(shards.get());
          payload_buffers.reserve(payload_buffer_count);
          for (size_t buffer_index = 0; buffer_index < payload_buffer_count; ++buffer_index) {
            payload_buffers.emplace_back(
              static_cast<const char *>(alk_gamestream_video_fec_frame_payload_buffer_data(shards.get(), buffer_index)),
              alk_gamestream_video_fec_frame_payload_buffer_bytes(shards.get(), buffer_index));
          }

          auto peer_address = session->video.peer.address();
          auto batch_info = platf::batched_send_info_t {
            static_cast<const char *>(alk_gamestream_video_fec_frame_prefix_data(shards.get(), 0)),
            shard_prefix_bytes,
            payload_buffers,
            shard_block_bytes,
            0,
          ...
          for (size_t x = 0; x < shard_count; ++x) {
            auto *inspect = (video_packet_raw_t *) alk_gamestream_video_fec_frame_shard_data(shards.get(), x);

            AlkGamestreamVideoShardWireParams shard_wire;
            alk_gamestream_video_shard_wire_params_init(&shard_wire);
            shard_wire.frame_index = packet->frame_index();
            shard_wire.stream_packet_index = static_cast<std::uint32_t>(lowseq + x);
            shard_wire.rtp_sequence_number = static_cast<std::uint32_t>(lowseq + x);
            shard_wire.rtp_timestamp = timestamp;
            shard_wire.block_index = static_cast<std::uint32_t>(blockIndex);
            shard_wire.block_count = static_cast<std::uint32_t>(fec_blocks_needed);
            shard_wire.shard_index = static_cast<std::uint32_t>(x);
            shard_wire.data_shards = static_cast<std::uint32_t>(data_shards);
            shard_wire.fec_percentage = static_cast<std::uint32_t>(shard_percentage);
            const auto [frame_start, frame_end] = video_shard_frame_boundary_flags(
              static_cast<std::uint32_t>(blockIndex),
              static_cast<std::uint32_t>(fec_blocks_needed),
              static_cast<std::uint32_t>(x),
              static_cast<std::uint32_t>(data_shards));
            shard_wire.frame_start = frame_start;
            shard_wire.frame_end = frame_end;
            alk_gamestream_video_packet_raw_prepare_post_fec_fields(inspect, &shard_wire);
            ...
            if (session->video.cipher) {
              ...
              auto *prefix = (video_packet_enc_prefix_t *) alk_gamestream_video_fec_frame_prefix_data(shards.get(), x);
            ...
            if (x - next_shard_to_send + 1 >= send_batch_size ||
              ...
              const auto send_result = alk_gamestream_host_media_udp_send_batch_or_each(
                ALK_TRANSPORT_CHANNEL_VIDEO_DATAGRAM,
                batch_info,
                next_shard_to_send,
                current_batch_size,
                [](platf::batched_send_info_t &info) {
                  return alk_gamestream_host_media_udp_product_send_batch(info);
                },
                [&](std::size_t shard_index) {
                  return alk_gamestream_host_media_udp_product_send_fragments(
                    static_cast<const char *>(alk_gamestream_video_fec_frame_prefix_data(shards.get(), shard_index)),
                    shard_prefix_bytes,
                    static_cast<const char *>(alk_gamestream_video_fec_frame_shard_data(shards.get(), shard_index)),
                    shard_block_bytes,
                    (uintptr_t) sock.native_handle(),
                    peer_address,
                    session->video.peer.port(),
              ...
              if (send_result.fallback_used) {
                BOOST_LOG(verbose) << "Falling back to unbatched send"sv;
              }
              if (!send_result.success) {
                BOOST_LOG(warning) << "Video media UDP adapter send failed after "
                                   << send_result.sent_datagrams << "/"sv << send_result.attempted_datagrams
                                   << " datagrams"sv;
              }
          ...
          BOOST_LOG(verbose) << "Sent Frame seq ["sv << packet->frame_index() << "] pts ["sv << timestamp
                             << "] shards ["sv << shard_count << "/"sv << shard_percentage << "%]"sv
                             << (frame_is_dupe ? " Dupe" : "")
                             << (packet->is_idr() ? " Key" : "")
  ...
  audioBroadcastThread(udp::socket &sock) {
    ...
    AlkMediaRsFecCodec *audio_fec_codec = alk_gamestream_rs_fec_codec_create();
    AlkMediaRsFecConfig audio_fec_config;
    alk_media_rs_fec_config_init(&audio_fec_config);
    audio_fec_config.data_shards = ALK_GAMESTREAM_AUDIO_DATA_SHARDS;
    audio_fec_config.fec_shards = ALK_GAMESTREAM_AUDIO_FEC_SHARDS;
    audio_fec_config.shard_bytes = crypto::cipher::round_to_pkcs7_padded(MAX_AUDIO_PACKET_SIZE);
    audio_fec_config.flags = ALK_MEDIA_RS_FEC_FLAG_GAMESTREAM_AUDIO_PARITY;
    if (audio_fec_codec == nullptr || !alk_media_rs_fec_codec_init(audio_fec_codec, &audio_fec_config)) {
      BOOST_LOG(error) << "Failed to initialize Alkaid GameStream audio RS-FEC codec"sv;
      if (audio_fec_codec != nullptr) {
        alk_media_rs_fec_codec_destroy(audio_fec_codec);
      }
      shutdown_event->raise(true);
      return;
    }
    ...
    // Audio RTP/FEC wire header preparation is owned by media.packetization/gamestream-rtp-fec.
    ...
    while (auto packet = packets->pop()) {
      ...
      *(std::uint32_t *) iv.data() = util::endian::big<std::uint32_t>(session->audio.avRiKeyId + sequenceNumber);
      ...
      auto bytes = encode_audio(audio_encryption_enabled, packet_data,
        shards_p[sequenceNumber % ALK_GAMESTREAM_AUDIO_DATA_SHARDS], iv, session->audio.cipher);
      ...
      if (!alk_gamestream_audio_packet_prepare(&audio_packet, sequenceNumber, timestamp, 0)) {
        BOOST_LOG(error) << "Alkaid GameStream audio RTP header prepare failed"sv;
        break;
      }
      ...
      try {
        const auto audio_send_result = alk_gamestream_host_media_udp_send_one(
          ALK_TRANSPORT_CHANNEL_AUDIO_DATAGRAM,
          [&]() {
            return alk_gamestream_host_media_udp_product_send_fragments(
              (const char *) &audio_packet,
              sizeof(audio_packet),
              (const char *) shards_p[sequenceNumber % ALK_GAMESTREAM_AUDIO_DATA_SHARDS],
              (size_t) bytes,
              (uintptr_t) sock.native_handle(),
              peer_address,
        ...
        if (!audio_send_result.success) {
          BOOST_LOG(warning) << "Audio media UDP adapter send failed";
        }
        record_network_transport_channel_datagram(session,
                                                  ALK_TRANSPORT_CHANNEL_AUDIO_DATAGRAM,
                                                  sizeof(audio_packet) + static_cast<std::size_t>(bytes),
                                                  audio_send_result.success);

        auto &fec_packet = session->audio.fec_packet;

        // generate parity shards at the end of the FEC block
        if ((sequenceNumber + 1) % ALK_GAMESTREAM_AUDIO_DATA_SHARDS == 0) {
          std::array<AlkMediaRsFecShard, ALK_GAMESTREAM_AUDIO_DATA_SHARDS> data_shards {};
          std::array<AlkMediaRsFecShard, ALK_GAMESTREAM_AUDIO_FEC_SHARDS> fec_shards {};
          for (auto i = 0; i < ALK_GAMESTREAM_AUDIO_DATA_SHARDS; ++i) {
            alk_media_rs_fec_shard_init(&data_shards[i]);
            data_shards[i].index = i;
            data_shards[i].flags = ALK_MEDIA_RS_FEC_SHARD_FLAG_DATA | ALK_MEDIA_RS_FEC_SHARD_FLAG_PRESENT;
            data_shards[i].payload = shards_p[i];
            data_shards[i].payload_length = bytes;
            data_shards[i].payload_capacity = audio_fec_config.shard_bytes;
          }
          for (auto i = 0; i < ALK_GAMESTREAM_AUDIO_FEC_SHARDS; ++i) {
            alk_media_rs_fec_shard_init(&fec_shards[i]);
            fec_shards[i].index = i;
            fec_shards[i].payload = shards_p[ALK_GAMESTREAM_AUDIO_DATA_SHARDS + i];
            fec_shards[i].payload_capacity = audio_fec_config.shard_bytes;
          }
          if (!alk_media_rs_fec_encode(audio_fec_codec, data_shards.data(), fec_shards.data())) {
            BOOST_LOG(error) << "Alkaid GameStream audio RS-FEC encode failed"sv;
            break;
          }

          for (auto x = 0; x < ALK_GAMESTREAM_AUDIO_FEC_SHARDS; ++x) {
            const auto block_base_sequence = static_cast<std::uint16_t>(sequenceNumber & ~(ALK_GAMESTREAM_AUDIO_DATA_SHARDS - 1));
            const auto block_base_timestamp = static_cast<std::uint32_t>(
              timestamp - (sequenceNumber % ALK_GAMESTREAM_AUDIO_DATA_SHARDS) * session->config.audio.packetDuration);
            if (!alk_gamestream_audio_fec_packet_prepare(&fec_packet,
                                                         static_cast<std::uint16_t>(sequenceNumber + x + 1),
                                                         static_cast<std::uint8_t>(x),
                                                         block_base_sequence,
                                                         block_base_timestamp,
                                                         0)) {
              BOOST_LOG(error) << "Alkaid GameStream audio FEC RTP header prepare failed"sv;
              break;
            }

            const auto audio_fec_send_result = alk_gamestream_host_media_udp_send_one(
              ALK_TRANSPORT_CHANNEL_AUDIO_DATAGRAM,
              [&]() {
                return alk_gamestream_host_media_udp_product_send_fragments(
                  (const char *) &fec_packet,
                  sizeof(fec_packet),
                  (const char *) shards_p[ALK_GAMESTREAM_AUDIO_DATA_SHARDS + x],
                  (size_t) bytes,
                  (uintptr_t) sock.native_handle(),
                  peer_address,
                  session->audio.peer.port(),
            ...
            if (!audio_fec_send_result.success) {
              BOOST_LOG(warning) << "Audio FEC media UDP adapter send failed";
            }
            record_network_transport_channel_datagram(session,
                                                      ALK_TRANSPORT_CHANNEL_AUDIO_DATAGRAM,
                                                      sizeof(fec_packet) + static_cast<std::size_t>(bytes),
                                                      audio_fec_send_result.success);
            BOOST_LOG(verbose) << "Audio FEC ["sv << (sequenceNumber & ~(ALK_GAMESTREAM_AUDIO_DATA_SHARDS - 1)) << ' ' << x << "] ::  send..."sv;
          }
        }
      }
      catch (const std::exception &e) {
  ...
  namespace session {
    ...
    join(session_t &session) {
      ...
      auto task = [&session]() {
        BOOST_LOG(fatal) << "Hang detected! Session failed to terminate in 10 seconds."
                         << " runtime=" << session.identity.runtime_id
                         << " userMode=" << (!is_running_as_system_user ? 1 : 0);
        logging::log_flush();
        if (!is_running_as_system_user) {
          BOOST_LOG(error) << "Suppressing session teardown debug trap in user-mode runtime="
                           << session.identity.runtime_id
                           << "; leaving process alive so later launches can recover";
      ...
      if (session.control_only) {
        ...
        if (--running_non_control_only_sessions == 0) {
          ...
          if (proc::proc.is_placebo_running()) {
            BOOST_LOG(info) << "Clearing placeholder app session after last runtime stopped"
                            << " runtime=" << session.identity.runtime_id
                            << " app=" << proc::proc.get_last_run_app_name();
            proc::proc.terminate();
          }
      ...
      BOOST_LOG(info) << "Startup timeline server stage=stream-start"
	      ...
	      session_runtime::apply_startup_path_preflight_snapshot(session.startup_path_evidence);
    ...
    std::shared_ptr<session_t>
      ...
      alk_transport_monitor_init(&session->transport_monitor);
      alk_transport_monitor_history_init(&session->transport_history);
      alk_transport_monitor_tuning_snapshot_init(&session->transport_tuning);
      alk_frame_interest_observation_init(&session->frame_interest_observation);
      ...
      session->active_transport_path = session_runtime::make_transport_path(session->startup_path_decision,
                                                                            session->startup_path_evidence);
      session->active_transport_path.state = session_runtime::transport_path_state_e::active;
      alk_handshake_selection_init(&session->handshake_selection);
      {
        AlkHandshakeOpenParams handshake_params;
        AlkHandshakeTransportCandidate transport_candidate;
        alk_handshake_open_params_init(&handshake_params);
        alk_handshake_transport_candidate_init(&transport_candidate);
        handshake_params.local_identity.role = ALK_SESSION_ROLE_HOST;
        handshake_params.peer_identity.role = ALK_SESSION_ROLE_CLIENT;
        handshake_params.local_identity.supported_handshake_modules =
          alk_gamestream_host_handshake_supported_modules(true);
        handshake_params.peer_identity.supported_handshake_modules =
          alk_gamestream_host_handshake_supported_modules(false);
        handshake_params.media_profile.width = static_cast<std::uint32_t>(std::max(config.monitor.width, 0));
        handshake_params.media_profile.height = static_cast<std::uint32_t>(std::max(config.monitor.height, 0));
        handshake_params.media_profile.fps = static_cast<std::uint32_t>(std::max(config.monitor.framerate, 0));
        alk_session_copy_string(handshake_params.media_profile.packetization_module_id,
                                sizeof(handshake_params.media_profile.packetization_module_id),
                                ALK_HANDSHAKE_DEFAULT_PACKETIZATION_MODULE_ID);
        alk_session_copy_string(handshake_params.media_profile.redundancy_module_id,
                                sizeof(handshake_params.media_profile.redundancy_module_id),
                                ALK_HANDSHAKE_DEFAULT_REDUNDANCY_MODULE_ID);
        handshake_params.launch_context.launch_session_id = launch_session.id;
        handshake_params.legacy_rtsp.connect_data = launch_session.control_connect_data;
        handshake_params.legacy_rtsp.control_port = net::map_port(CONTROL_PORT);
        ...
        transport_candidate.channel_mask = alk_gamestream_host_enet_control_channel_mask();
        ...
        transport_candidate.connect_data = launch_session.control_connect_data;
        alk_session_copy_string(transport_candidate.transport_module_id,
                                sizeof(transport_candidate.transport_module_id),
                                ALK_GAMESTREAM_ENET_TRANSPORT_MODULE_ID);
        alk_session_copy_string(transport_candidate.transport_module_version,
                                sizeof(transport_candidate.transport_module_version),
                                ALK_GAMESTREAM_ENET_TRANSPORT_MODULE_VERSION);
        copy_transport_path_to_alk(session->active_transport_path, transport_candidate.path);
        if (!launch_session.rtsp_route_host.empty()) {
          alk_session_copy_string(transport_candidate.path.remote_endpoint,
                                  sizeof(transport_candidate.path.remote_endpoint),
                                  launch_session.rtsp_route_host.c_str());
        }
        alk_handshake_open_params_add_transport_candidate(&handshake_params, &transport_candidate);
        alk_gamestream_host_handshake_select(&handshake_params, true, &session->handshake_selection);
      }
      log_startup_path_decision(session->identity.runtime_id,
                                "launch",
                                session->startup_path_evidence,
      ...
      BOOST_LOG(info) << "Alkaid runtime marker: slot=" << ALK_TRANSPORT_SLOT_ID
                      ...
                      << " adapter=" << ALK_GAMESTREAM_HOST_ENET_CONTROL_TRANSPORT_ADAPTER_ID
      ...
      BOOST_LOG(info) << "Alkaid handshake selection runtime=" << session->identity.runtime_id
                      << " slot=" << ALK_HANDSHAKE_SLOT_ID
                      << " module=" << session->handshake_selection.selected_handshake_module_id
                      << " transport=" << session->handshake_selection.selected_transport_module_id
                      << " packetization=" << session->handshake_selection.selected_packetization_module_id
                      << " redundancy=" << session->handshake_selection.selected_redundancy_module_id
                      << " path=" << session_runtime::transport_route_name(session->active_transport_path.route)
                      << " identity=" << session_runtime::li_path_identity_kind_name(session->active_transport_path.path_identity_kind)
                      << " startup=" << session_runtime::li_startup_class_name(session->active_transport_path.startup_class)
                      << " preflight=" << (session_runtime::startup_path_preflight_is_fresh(session->startup_path_evidence) ? 1 : 0)
                      << " preflightRttMs=" << session->startup_path_evidence.preflight_rtt_ms
                      << " preflightAgeMs=" << session->startup_path_evidence.preflight_age_ms
                      << " fallback="
                      << alk_handshake_fallback_code_name(session->handshake_selection.fallback_reason.code)
                      << " status=" << session->handshake_selection.status;
      ...
      if (enhanced_feedback_client && !strong_lan_fast_start) {
        ...
        const int startup_encoding_bitrate =
          startup_policy.allow_preflight_startup_lift ?
            computed_startup_bitrate :
        ...
        else if (startup_policy.allow_preflight_startup_lift &&
                 rtsp_seeded_bitrate > 0 &&
                 startup_encoding_bitrate > rtsp_seeded_bitrate) {
          BOOST_LOG(info) << "Remote-safe startup preflight lift runtime=" << session->identity.runtime_id
                          << " rtspSeed=" << rtsp_seeded_bitrate << " Kbps"
                          << " startup=" << encoding_bitrate << " Kbps"
                          << " fps=" << current_stream_fps
                          << " policy=" << startup_policy.reason
                          << " preflightRttMs=" << session->startup_path_evidence.preflight_rtt_ms
                          << " preflightAgeMs=" << session->startup_path_evidence.preflight_age_ms
                          << " ceilingEncoding=" << ceiling_encoding_bitrate << " Kbps";
        }
      ...
      util::buffer_t<char> shards { ALK_GAMESTREAM_AUDIO_TOTAL_SHARDS * max_block_size };
      util::buffer_t<uint8_t *> shards_p { ALK_GAMESTREAM_AUDIO_TOTAL_SHARDS };

      for (auto x = 0; x < ALK_GAMESTREAM_AUDIO_TOTAL_SHARDS; ++x) {
      ...
      std::memset(&session->audio.fec_packet, 0, sizeof(session->audio.fec_packet));
    ...
    get_all_sessions_info() {
      ...
      for (auto session_p : *broadcast_ref->control_server._sessions) {
        ...
        try {
          ...
          info.client_address = control_peer_label(session_p);
