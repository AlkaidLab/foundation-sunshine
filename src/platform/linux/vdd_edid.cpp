...
namespace vdd_edid {
  ...
  generate_virtual_display_edid(unsigned int width, unsigned int height, unsigned int refresh_hz, bool enable_hdr, const std::string &display_name) {
    ...
    // Screen size in cm, derived from a 96-DPI diagonal
    ...
    const auto h_size_cm = static_cast<unsigned int>((diagonal_inches * 2.54) / std::sqrt(1 + (1 / aspect_ratio) * (1 / aspect_ratio)));
    const auto v_size_cm = static_cast<unsigned int>(h_size_cm / aspect_ratio);
    ...
    if (enable_hdr) {
      ...
      // HDR Static Metadata Data Block: SDR+HDR+PQ, type 1, 1000/400/0.05 nits
      edid[offset] = 0xE6;
      edid[offset + 1] = 0x06;
      edid[offset + 2] = 0x07;
      edid[offset + 3] = 0x01;
