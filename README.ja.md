...
- **配信状態** - 有効と確認された Dolby Vision Profile 8.1 / 8.4 の出力表示と、全クライアント切断後に配信を終了するオプション
...
- **NVIDIA 画質向上：** 画質向上管理で RTX HDR または DLSS NR のコンポーネントを設定し、アプリごとに有効化します。DLSS NR は配信中にも調整できます。表示される SDR、HDR、Dolby Vision の出力は配信のネゴシエーションとデバイスの能力によって決まります。
- **USB 転送：** Windows ホストで USB/IP 転送コンポーネントが利用できる場合、ペアリング済みクライアントは実行時の USB 転送を設定できます。
