...
TEST(RtspBitrateAdjustmentTests, CapsConfiguredFecForLegacyClientsWithoutFeedback) {
  auto effective_fec = rtsp_stream::effective_stream_fec_percentage_for_client(100, 0);
  EXPECT_LE(effective_fec, 100);
...
TEST(RtspBitrateAdjustmentTests, FeedbackClientsKeepConfiguredBitrateAsVideoQualityCeiling) {
  constexpr int networkFeedbackFeatureFlag = 0x20;

  EXPECT_EQ(rtsp_stream::video_quality_ceiling_bitrate_kbps(
              20000,
              10,
              true,
              12,
              networkFeedbackFeatureFlag),
            20000);
}

TEST(RtspBitrateAdjustmentTests, LegacyClientsReserveFecInsideConfiguredBitrate) {
  EXPECT_EQ(rtsp_stream::video_quality_ceiling_bitrate_kbps(
              20000,
              100,
              false,
