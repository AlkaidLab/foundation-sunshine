...
> 开启后，所有客户端断开时会结束应用并恢复显示状态；关闭时保留现有的断线恢复行为。
...
- 自动终止请求使用一次性提交标记，两个会话几乎同时结束时最多提交一次。
...
实现不新增高频锁、网络请求或后台轮询；只在视频会话计数归零这一低频边沿提交一次异步终止请求，并在执行前重新确认状态。新的风险主要集中在“旧会话结束后新会话已启动”的竞态，已通过异步重新检查和一次性终止标记规避。
