...
4 个 20 ms 包提供约 80 ms 的单音源缓存，配合 2 帧 jitter buffer 已经能够吸收常见的短时网络抖动。把上限直接调大只能增加可缓存的数据量，不能改变发送速率高于播放速率时最终必然溢出的结果。

更大的队列还会带来两个代价：

- 首帧和恢复后的等待时间变长，麦克风延迟更明显；
- 队列中可能积累更多已经过时的数据，时间线错位后需要丢弃更多包才能恢复。

因此本设计保持 4 包上限，把持续溢出交给有界的时间线重锚处理。未来如果有真实设备数据证明网络抖动通常超过 80 ms，应基于测量结果调整 jitter buffer，而不是单独放大溢出队列。
