...
与用户沟通、撰写提交信息、PR 描述、代码注释、错误日志、文档时：

- **用中文正常表达**，句子是给人读的，不是给术语表读的。
- **不要堆英文行话 / 黑话**。能说"编码器"就不要说"encoder pipeline"；能说"丢帧"就不要说"frame drop 事件"；能说"鼠标灵敏度"就不要说"mouse sensitivity 参数"。
- 下列情况**才**保留英文原词：
  1. 代码符号（类名、函数名、变量名、配置键如 `mouse_sensitivity`、`virtual_sink`）
  2. 业界公认无标准中译的专有名词（HDR10、HLG、NVENC、VideoToolbox、RTSP、FFmpeg、Moonlight、Sunshine、BlackHole 等）
  3. 文件路径、命令、错误码
- **不要中英混杂造词**，例如"trigger 一次 refresh"、"check 一下 log"、"这个 feature 需要 enable"。要么整句中文（"触发一次刷新 / 查一下日志 / 这个功能需要启用"），要么纯代码/命令。
- 解释技术概念时先用中文把意思讲清楚，再在括号里附英文原词供交叉检索，而不是反过来。

此规则同样适用于在本仓库生成的任何 AI 辅助产物（PR 描述、issue 评论、提交信息、文档草稿）。
...
**仓库通过 git worktree 管理多个并行分支**：额外的工作副本位于 `.claude/worktrees/<slug>/`。不要假设当前目录是 `master` 检出 —— 在 worktree 内使用绝对路径，并注意此工作区相对 `master` 的分歧。
...
# 彻底清空 dist/ 再构建（切分支或改 vite 配置后使用）
npm run build-clean
...
> `prebuild` 钩子会在每次 `npm run build` 前自动运行 `src_assets/common/assets/web/scripts/extract-welcome-locales.js`，抽取欢迎屏 i18n 文案。修改欢迎屏或 locale 文件后，下一次 build 会自动同步。
...
# 反向同步：其他语言新增键合并回 en.json（批量外部翻译后使用）
...
测试使用 Google Test 框架，目录结构：
- `tests/tests_main.cpp`、`tests_common.h`、`tests_environment.h`、`tests_events.h`、`tests_log_checker.h`：测试框架入口与共享工具
- `tests/unit/`：按模块划分的单元测试（test_video.cpp / test_audio.cpp / test_network.cpp / test_mouse.cpp 等）
- `tests/unit/platform/`：平台特定测试
- 新测试需要在 `tests/CMakeLists.txt` 中注册

## 代码风格

- **C++**：`.clang-format` 已提供（LLVM 基底，4 空格缩进，`AllowShortBlocksOnASingleLine: Always`）。该文件**由组织级仓库集中管理**（见文件头注释），不要在本仓库内修改它 —— 本地改动会在下一次同步时被覆盖。
- **Vue/JS**：`.prettierrc.json` 提供配置，遵循项目已有缩进与引号习惯。
- **Python 脚本**：`.flake8` 提供配置。
...
仓库根目录同时保留 `CLAUDE.md`（本文件，面向 Claude Code）与 `AGENTS.md`（面向 Codex 及其他遵循 AGENTS.md 规范的代理）。两者内容几乎完全一致（仅标题不同）—— **修改其中一个时，必须同步另一个**，否则两侧代理拿到的指令会逐步漂移。
